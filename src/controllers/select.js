        ...
        for (let i = 0; i < Store.luckysheet_select_save.length; i++) {
            ...
            if (i == Store.luckysheet_select_save.length - 1) {
                ...
                luckysheet_count_show(
                    ...
                    Store.luckysheet_select_save[i]["height_move"], [r1, r2], [c1, c2]
                );
                //左上角选择区域框
...
function luckysheet_count_show(left, top, width, height, rowseleted, columnseleted) {
    ...
    if (rowl >= 4) {
        ...
        $("#luckysheet-row-count-show").css({ "left": leftv, "top": topv, "display": "block", "width": "16px" }).html("<div>" + rowl.toString().split("").join("</div><div>") + "</div><div>" + locale_info.row + "</div>");
    } else {
