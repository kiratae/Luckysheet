...
export default function luckysheetcreatedom(colwidth, rowheight, data, menu, title) {
    ...
    let addControll = '<button id="luckysheet-bottom-add-row" class="btn btn-secondary">' + locale_info.add + '</button><input id="luckysheet-bottom-add-row-input" type="text" class="form-control luckysheet-datavisual-config-input luckysheet-mousedown-cancel" placeholder="100"><span style="font-size: 14px;">' + locale_info.row + '</span><span style="font-size: 14px;color: #9c9c9c;">(' + locale_info.addLast + ')</span>';
    ...
    $("#luckysheet-cols-h-c").height((Store.columnHeaderHeight - 1));
    $("#luckysheet-left-top").css({ width: Store.rowHeaderWidth - 1.5, height: Store.columnHeaderHeight - 1 });
