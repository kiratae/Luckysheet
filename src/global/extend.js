...
function luckysheetextendtable(type, index, value, direction, sheetIndex) {
    ...
    if (calcChain != null && calcChain.length > 0) {
        for (let i = 0; i < calcChain.length; i++) {
            ...
            if (type == "row") {
                ...
                // [TK] custom
                if (d[calc_r][calc_c] && d[calc_r][calc_c].df == calc_funcStr) {
                    d[calc_r][calc_c].df = functionStr;
                }
    ...
    if (file.index == Store.currentSheetIndex) {
        jfrefreshgrid_adRC(
            d,
            cfg,
            "addRC", { "index": index, "len": value, "direction": direction, "rc": type1, "restore": false },
            newCalcChain,
            newFilterObj,
            newCFarr,
            newAFarr,
...
function luckysheetdeletetable(type, st, ed, sheetIndex) {
    ...
    if (file.index == Store.currentSheetIndex) {
        jfrefreshgrid_adRC(
            d,
            cfg,
            "delRC", { "index": st, "len": ed - st + 1, "rc": type1 },
            newCalcChain,
            newFilterObj,
            newCFarr,
            newAFarr,
...
function luckysheetDeleteCell(type, str, edr, stc, edc, sheetIndex) {
    ...
    if (file.index == Store.currentSheetIndex) {
        jfrefreshgrid_deleteCell(
            d,
            cfg, { type: type, 'str': str, 'edr': edr, 'stc': stc, 'edc': edc },
            newCalcChain,
            newFilterObj,
            newCFarr,
            newDataVerification,
